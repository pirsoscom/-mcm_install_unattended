...
echo "---------------------------------------------------------------------------------------------------------------------------"
        ...
        imageExists=$(docker pull $my_registry_name/multicluster-endpoint/agentoperator:APM_202003100816)
        if [[ ! $imageExists =~ "Image is up to date" ]];
        then
          ...
          ech o " ${wrench} ${GREEN}Image already exists. Skipping....${NC}"
        fi
        echo "----------------------------------------------------------------------------------------------------------------------------------------------------"
        echo " ${wrench} Loading ${CYAN}K9sMonitor Image${NC}"
        imageExists=$(docker pull $my_registry_name/multicluster-endpoint/k8-monitor:APM_202003092352)
        if [[ ! $imageExists =~ "Image is up to date" ]];
        then
            ...
            echo " ${wrench} ${GREEN}Image already exists. Skipping....${NC}"
        fi

        echo "----------------------------------------------------------------------------------------------------------------------------------------------------"
        echo " ${wrench} Loading ${CYAN}k8sdc-Operator Image${NC}"
        imageExists=$(docker pull $my_registry_name/multicluster-endpoint/k8sdc-operator:APM_202003092352)
        if [[ ! $imageExists =~ "Image is up to date" ]];
        then
        ...
        imageExists=$(docker pull $my_registry_name/multicluster-endpoint/reloader:202002170811-multi-arch)
        if [[ ! $imageExists =~ "Image is up to date" ]];
        then
        ...
        oc adm policy add-cluster-role-to-user cluster-admin $(oc whoami) --as=system:admin
...
system:serviceaccount:multicluster-endpoint:k8sdc-operator


oc create clusterrolebinding svcreg-k8sdc-operator-binding --clusterrole=cluster-admin \
 --serviceaccount=multicluster-endpoint:k8sdc-operator -n multicluster-endpoint
