...
# ---------------------------------------------------------------------------------------------------------------------------------------------------"
    ...
    # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        function checkRegistryCredentials() {
            ...
            DOCKER_PULL=$(docker pull $ENTITLED_REGISTRY/cp/icp-foundation/mcm-inception:$MCM_VERSION > /dev/null)

            echo "aaa$DOCKER_PULL"
   ...
   function checkRegistryCredentialsOK() {
        ...
        function checkComponentNotInstalled() {
            COMPONENT=$1
            echo "   ${magnifying} Check if ${CYAN}Component${NC} '${ORANGE}$COMPONENT${NC}' is already installed"
            ...
            if [[ $NUM_FOUND > 0 ]]; 
            then
                echo "    ${CYAN}Component${NC} '${ORANGE}$COMPONENT${NC}' is already installed"
    ...
    # ----^-----------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        function waitForPodsReady() {
            NAMESPACE=$1
            echo "${clock}   Waiting for Pods running in ${CYAN}Namespace${NC} ${ORANGE}$NAMESPACE${NC}."
        ...
        function waitForPodsReadyLabel() {
            NAMESPACE=$1
            LABEL=$2
            echo "   ${clock} Waiting for Pods running in ${CYAN}Namespace${NC} ${ORANGE}$NAMESPACE${NC} and with ${CYAN}Label${NC} ${ORANGE}$LABEL${NC}."
        ...
        function podsPending() {
            NAMESPACE=$1
            PODS_PENDING=$(kubectl get pods --field-selector=status.phase=Pending -n $NAMESPACE | grep -c "")
            if [[ "$PODS_PENDING" == "" ]]; then
                PODS_PENDING=0
            fi
            PODS_STATE=$(kubectl get pods -n $NAMESPACE | grep -E "Crash|Creat" | grep -c "")
            if [[ "$PODS_STATE" == "" ]]; then
                PODS_STATE=0
            fi
        ...
        function podsPendingLabel() {
            NAMESPACE=$1
            PODS_PENDING=$(kubectl get pods -l $LABEL --field-selector=status.phase=Pending -n $NAMESPACE | grep -c "")
            if [[ "$PODS_PENDING" == "" ]]; then
                PODS_PENDING=0
            fi
            PODS_STATE=$(kubectl get pods -l $LABEL -n $NAMESPACE | grep -E "Crash|Creat" | grep -c "")
            if [[ "$PODS_STATE" == "" ]]; then
                PODS_STATE=0
            fi
